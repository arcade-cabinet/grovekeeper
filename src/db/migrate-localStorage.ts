...
export function migrateFromLocalStorage(db: AppDatabase): boolean {
  ...
  const s: Record<string, unknown> =
    (parsed.state as Record<string, unknown>) ?? parsed;
...
function migrateResources(db: AppDatabase, s: Record<string, unknown>): void {
  ...
  const lifetimeResources = obj(s, "lifetimeResources") as Record<
    string,
    number
  > | null;
...
function migrateSettingsAndUpgrades(
  db: AppDatabase,
  s: Record<string, unknown>,
): void {
  ...
  const toolUpgradesData = obj(s, "toolUpgrades") as Record<
    string,
    number
  > | null;
  if (toolUpgradesData) {
