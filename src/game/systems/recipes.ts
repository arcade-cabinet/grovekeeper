 ...
 *   Tier 1 (Basic)        — Level 1-5   — common resources, small amounts
 *   Tier 2 (Intermediate) — Level 6-10  — more expensive, more useful
...
export interface SeedOutput {
  kind: "seed";
  /** Species pool that can drop. Empty array = any common species. */
  speciesPool: string[];
  amount: number;
}
...
export interface EffectOutput {
  ...
  /** Numeric magnitude (interpretation depends on effect type). */
  magnitude: number;
  /** Duration in seconds. 0 = instant, -1 = permanent. */
  durationSec: number;
}
...
export interface Recipe {
  ...
  /** Optional craft duration in seconds (0 = instant). */
  craftTime?: number;
  /** templateId of required nearby structure. */
...
/** Return a shallow copy of all recipes. */
...
/**
 * Calculate the total input cost of a recipe as a resource map.
 * Useful for UI display and cost previews.
 */
