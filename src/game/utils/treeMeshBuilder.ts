...
const SPECIES_PROFILES: Record<string, SpeciesProfile> = {
  "white-oak": {
    ...
    sps: { trunkHeight: 1.8, forks: 3, boughs: 2, branches: 8, leavesOnBranch: 5, forkAngle: 1.0, forkRatio: 0.7, bowHeight: 2 },
  },
  "weeping-willow": {
    bark: "smooth", leaf: "broad",
    barkTint: [0.65, 0.55, 0.42],
    leafTint: [0.4, 0.73, 0.41],
    sps: { trunkHeight: 2.0, forks: 4, boughs: 2, branches: 15, leavesOnBranch: 10, forkAngle: 1.3, forkRatio: 0.65, bowHeight: 5.2 },
  },
  "elder-pine": {
    bark: "rugged", leaf: "feathery",
  ...
  "baobab": {
    bark: "thick", leaf: "feathery",
    barkTint: [0.58, 0.45, 0.32],
    leafTint: [0.33, 0.54, 0.18],
    sps: { trunkHeight: 2.5, forks: 4, boughs: 1, branches: 4, leavesOnBranch: 3, forkAngle: 1.6, forkRatio: 0.5, trunkTaper: 0.3, bowHeight: 1 },
  },
  // Prestige species
  "crystal-oak": {
    bark: "birch", leaf: "broad",
...
export function buildSpeciesTreeMesh(
  ...
  const barkMat = getOrCreateBarkMaterial(scene, profile.bark, barkTex, profile.barkTint);
  ...
  const leafTint = resolveLeafTint(
    profile, speciesId, season, species?.evergreen ?? false, rng, nightTime,
  );
  const leafMat = getOrCreateLeafMaterial(scene, `${profile.leaf}_${season ?? "default"}`, leafTex, leafTint);
