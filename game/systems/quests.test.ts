...
describe("Quest System", () => {
  describe("Goal Pools", () => {
    it("collection pool has goals", () => {
      expect(GOAL_POOLS.collection.length).toBeGreaterThan(0);
    });

    it("exploration pool has goals", () => {
  ...
  describe("generateQuest", () => {
    it("returns a quest with xp rewards", () => {
    ...
    it("quest rewards may include resources", () => {
      ...
      for (let i = 0; i < 20; i++) {
        const quest = generateQuest("medium", "spring", new Set(), "test", i);
    ...
    it("exploration goals can appear in easy quests", () => {
      ...
      for (let i = 0; i < 200; i++) {
        const quest = generateQuest("easy", "spring", new Set(), "test", i);
    ...
    it("accepts worldSeed for deterministic generation", () => {
      const quest = generateQuest("easy", "spring", new Set(), "Gentle Mossy Hollow", 0);
      expect(quest).not.toBeNull();
      expect(quest!.goals.length).toBe(1);
    });
  });
  ...
  describe("generateDailyQuests", () => {
    ...
    it("accepts worldSeed parameter", () => {
      const quests = generateDailyQuests("spring", 15, new Set(), "Gentle Mossy Hollow");
      expect(quests.length).toBe(4);
    });
  });
