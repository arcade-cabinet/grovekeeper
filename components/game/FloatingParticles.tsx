...
function WeatherParticle({
  ...
  const leftPct = useMemo(() => {
    const rng = scopedRNG("wp-left", worldSeed, index);
    return (index / total) * 100 + rng() * 5;
  }, [index, total, worldSeed]);
  const startDelay = useMemo(() => {
    const rng = scopedRNG("wp-delay", worldSeed, index + 2000);
    return rng() * lifetimeMs;
  }, [index, lifetimeMs, worldSeed]);
  ...
  useEffect(() => {
    translateY.value = withDelay(
      ...
      withRepeat(
        withSequence(
          ...
          withTiming(travelPx, { duration: lifetimeMs, easing: Easing.linear }),
        ),
        -1,
        false,
        () => {},
    ...
    translateX.value = withDelay(
      startDelay,
      withRepeat(withTiming(windDrift, { duration: lifetimeMs }), -1, false),
    );
    opacity.value = withDelay(
      startDelay,
      withRepeat(
        withSequence(
          withTiming(intensity * 0.6, { duration: lifetimeMs * 0.15 }),
          withTiming(intensity * 0.3, { duration: lifetimeMs * 0.7 }),
          withTiming(0, { duration: lifetimeMs * 0.15 }),
        ),
        -1,
        false,
        () => {},
      ...
      style={[
        {
          ...
          left: `${leftPct}%`,
          top: -10,
          width: isSphere ? dotSize : 1,
          height: isSphere ? dotSize : 14,
          backgroundColor: color,
          borderRadius: isSphere ? 50 : 1,
        },
        animatedStyle,
      ]}
    />
...
export function WeatherParticlesLayer() {
  ...
  const activeEmitters: Partial<
    Record<
      WeatherPType,
      { color: string; size: number; lifetime: number; gravity: number; maxParticles: number }
    >
  > = {};
  ...
  const entries = Object.entries(activeEmitters) as [
    WeatherPType,
    NonNullable<(typeof activeEmitters)[WeatherPType]>,
  ][];
  ...
  return (
    <View className="absolute inset-0" pointerEvents="none" style={{ zIndex: 9997 }}>
      {entries.flatMap(([pType, cfg]) => {
        const count = computeDisplayParticleCount(
        ...
        return Array.from({ length: count }, (_, i) => (
          <WeatherParticle
            ...
            color={cfg.color}
            size={cfg.size}
            lifetimeMs={cfg.lifetime * 1000}
            gravity={cfg.gravity}
            windDrift={windDrift}
            intensity={intensity}
            worldSeed={`${worldSeed}-${pType}`}
            isSphere={pType === "snow"}
          />
        ));
      })}
