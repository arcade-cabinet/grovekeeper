...
describe("resolveEntityDisplayName (Spec §15)", () => {
  it("returns npc.name when NPC component is present", () => {
    expect(
      resolveEntityDisplayName({ name: "Elder Rowan" }, { spiritId: "spirit-1" }, "entity_42"),
    ).toBe("Elder Rowan");
  });

  it("prefers NPC name over spirit id", () => {
    expect(resolveEntityDisplayName({ name: "The Merchant" }, { spiritId: "spirit-3" })).toBe(
      "The Merchant",
    );
  });

  it("returns spirit label when no NPC component", () => {
    expect(resolveEntityDisplayName(undefined, { spiritId: "spirit-1" }, "entity_42")).toBe(
      "Spirit: spirit-1",
    );
  });

  it("uses entityId as fallback when neither npc nor spirit", () => {
    expect(resolveEntityDisplayName(undefined, undefined, "entity_42")).toBe("entity_42");
  });
  ...
  it("returns Unknown when entityId is also undefined", () => {
    expect(resolveEntityDisplayName(undefined, undefined, undefined)).toBe("Unknown");
  });
});
